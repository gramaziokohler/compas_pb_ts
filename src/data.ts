...
export const protobufRegistry = new Map<
  any,
  { encode: Function; decode: Function; name: string; protobufPackage: string }
>([
  [
    BeamData,
    {
      encode: BeamData.encode,
      decode: BeamData.decode,
      name: "BeamData",
      protobufPackage: "compas_timber_pb.data",
    },
  ],
  [
    BTLxProcessingData,
    {
      encode: BTLxProcessingData.encode,
      decode: BTLxProcessingData.decode,
      name: "BTLxProcessingData",
      protobufPackage: "compas_timber_pb.data",
    },
  ],
...
export function packAnyData<T>(obj: T, iface: any): AnyData {
  ...
  if (!entry)
    throw new Error(`No protobuf registry entry for interface: ${iface}`);
